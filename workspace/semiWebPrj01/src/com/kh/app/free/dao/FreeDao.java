...
public class FreeDao {
	...
	public List<FreeVo> selectList(Connection conn, PageVo pageVo) throws Exception {
		...
		// SQL (close)
		String sql = "SELECT FREE_NO, TITLE FROM F_BO WHERE DEL_YN = 'N'";
	...
	public int write(Connection conn, FreeVo vo) throws Exception {
		...
		// SQL (close)
		String sql = "INSERT INTO F_BO (FREE_NO , TITLE , CONTENT, WRITER,ENROLL_DATE,DEL_YN) VALUES ('3', '3' , '3','111',SYSDATE,'N')";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public FreeVo selectOne(Connection conn, String no) throws Exception {
		...
		// SQL
		String sql = "SELECT B.FREE_NO , B.TITLE , B.CONTENT, A.CHANGE_NAME FROM F_BO B JOIN F_ATTACHMENT A ON (B.FREE_NO = A.REF_BOARD_NO) WHERE B.FREE_NO = ? AND B.DEL_YN = 'N'";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public int insertAttachment(Connection conn, FreeAttachmentVo atVo) throws Exception {
		...
		// SQL
		String sql = "INSERT INTO F_ATTACHMENT (NO, ORIGIN_NAME, CHANGE_NAME, REF_BOARD_NO) VALUES('1' , ? , ? , '3')";
		PreparedStatement pstmt = conn.prepareStatement(sql);
